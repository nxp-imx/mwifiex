...
#define PERFMATRIX_BUFFER_SIZE 4096
#ifdef STA_SUPPORT
...
/**
 *  @brief Update perfmatrix buffer with new data
 *
 ...
 *  @return         N/A
 */
void woal_update_perfmatrix_buffer(moal_private *priv, const char *data)
{
	unsigned long flags;
	t_u32 data_len;
	char timestamp[64];
	...
	ENTER();
	if (!priv || !data || !priv->perfmatrix_data.buffer ||
	    priv->perfmatrix_data.buffer_size == 0) {
		PRINTM(MERROR, "perfmatrix: Unable to update");
		goto done;
	}

	/* Generate timestamp */
	ktime_get_real_ts64(&ts);
	timestamp_len = snprintf(timestamp, sizeof(timestamp), "[%lld.%06ld] ",
				 ts.tv_sec, ts.tv_nsec / 1000);
	if (timestamp_len < 0 || timestamp_len >= sizeof(timestamp)) {
		PRINTM(MERROR, "perfmatrix: Failed to generate timestamp\n");
		goto done;
	}
	/* Calculate total data length for pre-validation */
	data_len = timestamp_len + strlen(data) + 1;

	/* Pre-check if data will fit */
	if (data_len >= priv->perfmatrix_data.buffer_size) {
		PRINTM(MERROR,
		       "perfmatrix: Data too large (%u bytes, buffer size %u)\n",
		       data_len, priv->perfmatrix_data.buffer_size);
		goto done;
	}
	spin_lock_irqsave(&priv->perfmatrix_data.buffer_lock, flags);
	/* Clear buffer completely (OVERWRITE mode) */
	memset(priv->perfmatrix_data.buffer, 0,
	       priv->perfmatrix_data.buffer_size);
	priv->perfmatrix_data.buffer_len = 0;
	written = snprintf(priv->perfmatrix_data.buffer,
			   priv->perfmatrix_data.buffer_size, "%s%s", timestamp,
			   data);
	if (written < 0) {
		PRINTM(MERROR, "perfmatrix: snprintf failed with error %d\n",
		       written);
		priv->perfmatrix_data.buffer_len = 0;
	} else if (written >= priv->perfmatrix_data.buffer_size) {
		PRINTM(MWARN,
		       "perfmatrix: Data truncated (needed %d bytes, available %u)\n",
		       written, priv->perfmatrix_data.buffer_size);
		priv->perfmatrix_data.buffer_len =
			priv->perfmatrix_data.buffer_size - 1;
	} else {
		priv->perfmatrix_data.buffer_len = written;
		PRINTM(MINFO, "perfmatrix: Successfully updated %d bytes\n",
		       written);
	}
	priv->perfmatrix_data.last_update = jiffies;
	spin_unlock_irqrestore(&priv->perfmatrix_data.buffer_lock, flags);
...
/** @brief proc read function for perfmatrix
 *
 *  @param sfp     pointer to seq_file structure
 *  @param data    proc data
 *
 *  @return        Number of output data or MLAN_STATUS_FAILURE
 */
static int woal_perfmatrix_read(struct seq_file *sfp, void *data)
{
	moal_private *priv = (moal_private *)sfp->private;
	unsigned long flags;
	...
	spin_lock_irqsave(&priv->perfmatrix_data.buffer_lock, flags);
	if (priv->perfmatrix_data.buffer_len > 0) {
		seq_printf(sfp, "%.*s", priv->perfmatrix_data.buffer_len,
			   priv->perfmatrix_data.buffer);
	} else {
		seq_printf(sfp, "No perfmatrix data available\n");
	}
	spin_unlock_irqrestore(&priv->perfmatrix_data.buffer_lock, flags);
	MODULE_PUT;
	LEAVE();
...
/*  @brief Proc open function for performance matrix
 *
 *  @param inode     pointer to inode
 *  @param file       file pointer
 *
 *  @return        number of data
 */
static int woal_perfmatrix_proc_open(struct inode *inode, struct file *file)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 16, 0)
	return single_open(file, woal_perfmatrix_read, pde_data(inode));
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	return single_open(file, woal_perfmatrix_read, PDE_DATA(inode));
#else
	return single_open(file, woal_perfmatrix_read, PDE(inode)->data);
#endif
}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0)
static const struct proc_ops perfmatrix_proc_fops = {
	.proc_open = woal_perfmatrix_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
static const struct file_operations perfmatrix_proc_fops = {
	.owner = THIS_MODULE,
	.open = woal_perfmatrix_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
void woal_debug_entry(moal_private *priv)
{
	...
	priv->perfmatrix_data.buffer_size = PERFMATRIX_BUFFER_SIZE;
	priv->perfmatrix_data.buffer =
		kzalloc(PERFMATRIX_BUFFER_SIZE, GFP_KERNEL);
	if (!priv->perfmatrix_data.buffer) {
		PRINTM(MERROR, "Failed to allocate perfmatrix buffer\n");
		LEAVE();
		return;
	}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 26)
	r = proc_create_data("perfmatrix", 0644, priv->proc_entry,
			     &perfmatrix_proc_fops, priv);
	if (r == NULL)
#else
	r = create_proc_entry("perfmatrix", 0644, priv->proc_entry);
	if (r) {
		r->data = priv;
		r->proc_fops = &perfmatrix_proc_fops;
	} else
#endif
	{
		PRINTM(MMSG, "Fail to create proc perfmatrix entry\n");
		LEAVE();
		return;
	}
...
void woal_debug_remove(moal_private *priv)
{
	...
	remove_proc_entry("perfmatrix", priv->proc_entry);
	kfree(priv->perfmatrix_data.buffer);
	priv->perfmatrix_data.buffer = NULL;
