	...
	mlan_ds_11ax_llde_pkt_filter_cmd *param_buf)
{
	...
	above,
	respbuf will contain llde cfg parameters only which will be later parsed
	via parse_arguments() */
	if (llde_pkt_filter_len > 0) {
				      ...
				      t_u32 respbuflen)
{
	...
	if ((int)strlen(respbuf) == header_len) {
		...
		case 1: /* enable */
			...
			break;
		case 2: /* reset */
			woal_print_linkstats_info(priv, MTRUE, MTRUE);
			moal_memcpy_ext(priv->phandle, respbuf, (t_u8 *)data,
					sizeof(data), respbuflen);
			ret = sizeof(data);
			break;
		case 3: /* get */
			woal_print_linkstats_info(priv, MFALSE, MTRUE);
				...
				t_u32 respbuflen, t_u8 bBSSID)
{
	...
	if (MLAN_STATUS_SUCCESS != woal_disconnect(priv, MOAL_IOCTL_WAIT, NULL,
						   DEF_DEAUTH_REASON_CODE)) {
		PRINTM(MERROR, " Error when disconnecting\n");
	}
			       ...
			       t_u32 respbuflen)
{
	...
	if (user_data_len >= 1 && user_data_len <= 15) {
		...
		offset = strlen(CMD_NXP) + strlen(PRIV_CMD_HSSETPARA);
		copy_len = CMD_BUF_LEN - offset;
		if (copy_len > 0) {
			memcpy(buf, respbuf + offset, copy_len - 1);
			buf[copy_len - 1] = '\0';
		} else {
			kfree(buf);
			goto done;
					    ...
					    t_u8 support_ecsa)
{
	...
	if (((band == BAND_5GHZ) && !woal_5g_support(phandle->fw_bands)) ||
