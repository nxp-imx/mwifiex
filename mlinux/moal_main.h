...
#ifdef MAC80211_SUPPORT
/** fw cap info mac80211 */
#define FW_CAPINFO_MAC80211 MBIT(26)
#endif
...
/** perfmatrix proc data structure */
typedef struct _perfmatrix_data {
	/** Pointer to moal_private structure */
	void *priv;
	/** Buffer to store perfmatrix data */
	char *buffer;
	/** Buffer size */
	t_u32 buffer_size;
...
/** Function to update perfmatrix buffer */
...
#define CUS_EVT_PERFMATRIX "EVENT=PERFMATRIX_UPDATE"
...
#ifdef MAC80211_SUPPORT
/** Driver mode MAC80211 bit */
#define DRV_MODE_MAC80211 MBIT(6)
/** Maximum MAC80211 BSS */
#define MAX_MAC80211_BSS 1
/** Default MAC80211 BSS */
#define DEF_MAC80211_BSS 1
#endif
...
/** number of max deauth events used in print linkstats cmd*/
#define MAX_DEAUTH_EVENTS 32
...
typedef struct _moal_priv_linkstats {
	...
	t_u8 deauth_count;
	/** deauth reason code */
	t_u16 deauth_reason[MAX_DEAUTH_EVENTS];
	/** deauth mac addr */
	t_u8 deauth_mac[MAX_DEAUTH_EVENTS][MLAN_MAC_ADDR_LENGTH];
} moal_priv_linkstats;
...
struct _moal_private {
	...
	/** perfmatrix proc data */
	perfmatrix_data perfmatrix_data;
	...
	BOOLEAN mac80211_started;
	BOOLEAN mac80211_registered;
	struct list_head sta_list;
	spinlock_t sta_list_lock;
	struct workqueue_struct *beacon_update_workqueue;
	struct work_struct beacon_update_work;
	BOOLEAN mac80211_mode_monitor;
#endif
#ifdef UAP_SUPPORT
#if defined(UAP_CFG80211) || defined(STA_CFG80211)
	t_u8 multi_ap_flag;
...
#ifndef DUMP_TO_PROC
t_void woal_print_firmware_dump_via_file(moal_handle *phandle,
					 char *fwdp_fname);
#endif /* DUMP_TO_PROC */
...
void woal_fill_mlan_buffer(moal_private *priv, mlan_buffer *pmbuf,
			   struct sk_buff *skb);
#ifdef MAC80211_SUPPORT
moal_private *woal_add_mac80211_interface(moal_handle *handle, t_u8 bss_num,
					  t_u8 bss_type);
void woal_remove_mac80211_interface(moal_private *priv);
mlan_status moal_recv_event_mac80211(moal_private *priv, pmlan_event pmevent);
mlan_status moal_send_packet_complete_mac80211(t_void *pmoal,
					       pmlan_buffer pmbuf,
					       mlan_status status);
mlan_status moal_recv_packet_mac80211(t_void *pmoal, pmlan_buffer pmbuf);
extern int mac80211_rate_adapt;
#endif
...
#if KERNEL_VERSION(3, 14, 0) <= CFG80211_VERSION_CODE
void woal_cfg80211_vendor_event_plinkstats(moal_private *priv);
#endif
