...
#define PLSTATS_FILTER_DEAUTH_REASON MBIT(15)
...
mlan_status woal_update_drv_tbl(moal_handle *handle, int drv_mode_local)
{
	...
	int max_mac80211_bss = MAX_MAC80211_BSS;
#endif
...
#ifdef MAC80211_SUPPORT
	if (drv_mode_local & DRV_MODE_MAC80211)
		intf_num += max_mac80211_bss;
...
#ifdef MAC80211_SUPPORT
	if (drv_mode_local & DRV_MODE_MAC80211) {
		for (j = 0; j < max_mac80211_bss; j++) {
			if (i >= (int)intf_num)
				break;
			bss_tbl[i].bss_type = MLAN_BSS_TYPE_MAC80211;
			bss_tbl[i].frame_type = MLAN_DATA_FRAME_TYPE_802_11;
			bss_tbl[i].active = MTRUE;
			bss_tbl[i].bss_priority = 0;
			bss_tbl[i].bss_num = j;
			bss_tbl[i].bss_virtual = MFALSE;
...
#else
	// coverity[misra_c_2012_rule_17_12_violation:SUPPRESS]
...
#ifdef MAC80211_SUPPORT
	if (bss_type == MLAN_BSS_TYPE_MAC80211) {
		LEAVE();
		return woal_add_mac80211_interface(handle, bss_index, bss_type);
	}
#endif
...
#define MAX_WMM_QUEUE 4
	...
	if (bss_type == MLAN_BSS_TYPE_STA
		...
		if (MLAN_STATUS_SUCCESS != woal_init_sta_dev(dev, priv)) {
			// coverity[misra_c_2012_rule_20_12_violation:SUPPRESS]
...
#ifdef MAC80211_SUPPORT
	if (priv && priv->bss_type == MLAN_BSS_TYPE_MAC80211) {
		woal_remove_mac80211_interface(priv);
		LEAVE();
		return;
	}
#endif
...
void woal_init_priv(moal_private *priv, t_u8 wait_option)
{
	...
	priv->plinkstats.deauth_count = 0;
...
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
#ifdef MAC80211_SUPPORT
	if (priv->bss_type == MLAN_BSS_TYPE_MAC80211 &&
	    priv->mac80211_registered && priv->mac80211_mode_monitor) {
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_net_monitor(priv, wait_option, MFALSE, 0, NULL)) {
			PRINTM(MERROR, "%s: stop net monitor failed \n",
			       __func__);
			/* Even if stop net monitor is failed still continue to
			 * clean the kernel related config otherwise it leads to
...
t_void woal_store_firmware_dump(moal_handle *phandle, mlan_event *pmevent)
{
	...
	if (type == DUMP_TYPE_ENDE) {
		...
		{
			t_u32 fwdump_len = 0;
			fwdump_len = (t_u32)(phandle->fw_dump_len & 0xffffffff);

			PRINTM(MFWDP_D,
			       "Read FW dump from fw_dump_buf and print it on dmesg log!\n");
			woal_print_firmware_dump(fwdump_len,
						 phandle->fw_dump_buf);
		}
...
t_void woal_store_firmware_dump(moal_handle *phandle, mlan_event *pmevent)
{
	...
	if (type == DUMP_TYPE_ENDE) {
#ifdef FWDUMP_VIA_PRINT
		woal_print_firmware_dump_via_file(
			phandle,
			(phandle->fwdump_fname ? phandle->fwdump_fname :
						 fwdump_fname));
#endif /*FWDUMP_VIA_PRINT*/
		PRINTM(MMSG, "==== FW DUMP END: %ld bytes ====\n",
		       (long int)phandle->fw_dump_len);
...
#ifdef FWDUMP_VIA_PRINT
#ifndef DUMP_TO_PROC
t_void woal_print_firmware_dump_via_file(moal_handle *phandle, char *fwdp_fname)
{
	struct file *pfile_fwdump = NULL;
	loff_t pos = 0;
	t_u8 *pbuf = NULL;
	t_u32 fwdump_len = 0, ret = 0;

	ENTER();
	if ((phandle->fw_dump_len == 0) || (fwdp_fname == NULL)) {
		PRINTM(MFWDP_D, "Failed to print FW dump from file(%s)!\n",
		       fwdp_fname);
	} else {
		pos = 0;
		fwdump_len = (t_u32)(phandle->fw_dump_len & 0xffffffff);
		...
		if (ret != MLAN_STATUS_SUCCESS || !pbuf) {
			...
			if (ret <= 0) {
				...
				PRINTM(MFWDP_D,
				       "Read FW dump from file(%s) and print it on dmesg log!\n",
				       fwdp_fname);
				woal_print_firmware_dump(fwdump_len, pbuf);
			}
...
t_void woal_print_firmware_dump(t_u32 fwdump_len, t_u8 *pbuf)
{
	int i;

	ENTER();

	if ((fwdump_len == 0) || (pbuf == NULL)) {
		PRINTM(MFWDP_D, "Failed to print FW dump from fw_dump_buf!\n");
	} else {
		PRINTM(MFWDP_D, "===== FW Dump To Console START=====\n");
		for (i = 0; ((i < fwdump_len) && ((i + 15) < fwdump_len));
		     i += 16) {
			PRINTM(MFWDP_D,
			       "[FW Dump] %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",
			       pbuf[i], pbuf[i + 1], pbuf[i + 2], pbuf[i + 3],
			       pbuf[i + 4], pbuf[i + 5], pbuf[i + 6],
			       pbuf[i + 7], pbuf[i + 8], pbuf[i + 9],
			       pbuf[i + 10], pbuf[i + 11], pbuf[i + 12],
			       pbuf[i + 13], pbuf[i + 14], pbuf[i + 15]);
			usleep_range(10, 20);
		}
		...
		PRINTM(MFWDP_D, "FW Dump buffer length = %ld\n",
		       (long int)fwdump_len);
	}
...
static int woal_dump_priv_drv_info(moal_handle *handle, t_u8 *buf)
{
	...
	for (index = 0; index < MIN(handle->priv_num, MLAN_MAX_BSS_NUM);
		...
		if (priv) {
#ifdef MAC80211_SUPPORT
			/* MAC80211 interface does not use WMM queues
			 * and there is no netdev */
			if (priv->bss_type == MLAN_BSS_TYPE_MAC80211)
				continue;
#endif
 ...
 */
static int woal_dump_event_hex(moal_handle *phandle, t_u8 *buf,
			       mlan_event *pmevent)
{
	char *ptr = NULL;
	moal_private *priv = NULL;
	...
	t_u8 *tempstore = NULL;
	t_u16 command_size;
	t_u16 command_id = HostCmd_CMD_CFG_DATA;
	int ret;
	ENTER();
	command_size = pmevent->event_len - 4 + sizeof(HostCmd_DS_GEN);
	ret = moal_vmalloc(phandle, command_size, &tempstore);
	if ((ret != MLAN_STATUS_SUCCESS) || !tempstore) {
		PRINTM(MERROR, "Error: vmalloc buf failed!\n");
		LEAVE();
		return 0;
	}
	memset(tempstore, 0, command_size);
	if (!phandle || !buf) {
	...
	moal_get_system_time(phandle, &sec, &usec);
	ptr = (char *)buf;
	ptr += sprintf(ptr, "hostcmd_8f=={\n");
	PRINTM(MINFO, "command_size  %d command_id %d", command_size,
	       command_id);
	moal_memcpy_ext(phandle, tempstore, &command_id, sizeof(t_u16),
			sizeof(t_u16));
	moal_memcpy_ext(phandle, tempstore + 2, &command_size, sizeof(t_u16),
			sizeof(t_u16));
	moal_memcpy_ext(phandle, tempstore + sizeof(HostCmd_DS_GEN),
			pmevent->event_buf + 4, pmevent->event_len - 4,
			(command_size - sizeof(HostCmd_DS_GEN)));
	ptr += woal_save_hex_dump(
		ROW_SIZE_16, tempstore,
		pmevent->event_len - 4 + sizeof(HostCmd_DS_GEN), MFALSE, ptr);
	ptr += sprintf(ptr, "}\n");
	if (tempstore)
		moal_vfree(phandle, tempstore);
	LEAVE();
...
t_void woal_store_dpd_log(moal_handle *phandle, mlan_event *pmevent)
{
	...
	memset(file_path, 0, sizeof(file_path));
	ret = moal_vmalloc(phandle, pmevent->event_len * 5, &buf);
	if ((ret != MLAN_STATUS_SUCCESS) || !buf) {
		PRINTM(MERROR, "Error: vmalloc buf failed!\n");
		goto done;
	}
	memset(buf, 0, pmevent->event_len * 5);
	if (dpd_data_cfg)
		sprintf(file_path, "%s%s", DEF_FW_PATH, dpd_data_cfg);
	pfile_dpdlog =
		filp_open(file_path, O_CREAT | O_WRONLY | O_APPEND, 0644);
	if (IS_ERR(pfile_dpdlog)) {
		PRINTM(MERROR, "Cannot create dpdlog file %s\n", file_path);
		LEAVE();
		return;
	}
...
#endif
	PRINTM(MINFO,
	       "Save Event as  0x008f, cmd len=%d file len=%d to file %s\n",
	       pmevent->event_len, len, file_path);
...
#define MAX_LINK_STATS_BUF_SIZE 4096
/**
 * @brief               This function send link statistics event to userspace
 *
 * @param priv          a pointer to moal_private structure
 *
 * @return              N/A
 */
void woal_send_linkstats_event(moal_private *priv)
{
	char *stats_buf = NULL;
	char *ptr = NULL;
	int i;
	...
	stats_buf = kmalloc(MAX_LINK_STATS_BUF_SIZE, GFP_ATOMIC);
	if (!stats_buf)
		return;

	memset(stats_buf, 0, MAX_LINK_STATS_BUF_SIZE);
	ptr = stats_buf;

	ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE, CUS_EVT_PRINT_LINK_STATS);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_TX_BYTES)
		ptr += snprintf(
			ptr, MAX_LINK_STATS_BUF_SIZE,
			"\nNum_tx_bytes = %lu, \t Num_tx_packets = %lu\n",
			priv->plinkstats.tx_bytes, priv->plinkstats.tx_packets);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_RX_BYTES)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
	...
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_CHLOAD)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				"Channel Load = %d%%, noise = %d\n",
				priv->plinkstats.chload,
				priv->plinkstats.noise);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_CHANNEL)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE, "Channel = %d\n",
				priv->plinkstats.channel);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_REGION) {
		char *region_str =
			region_code_2_string(priv->plinkstats.region_code);
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE, "Region = %s\n",
				region_str ? region_str : "Unknown");
	}
	...
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_TX_DROPPED)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				"Tx dropped packets = %lu\n",
				priv->plinkstats.tx_dropped);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_DEAUTH_TX)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				"Deauth TX event count = %lu\n",
				priv->plinkstats.num_evt_deauth_tx);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_DEAUTH_RX)
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				"Deauth RX event count = %lu\n",
				priv->plinkstats.num_evt_deauth_rx);

	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_DEAUTH_REASON) {
		if (priv->plinkstats.deauth_count) {
			int idx;
			for (idx = 0; idx < priv->plinkstats.deauth_count;
			     idx++) {
				ptr += snprintf(
					ptr, MAX_LINK_STATS_BUF_SIZE,
					"Station deauthed " MACSTR
					", reason code = 0x%x\n",
					MAC2STR(priv->plinkstats
							.deauth_mac[idx]),
					priv->plinkstats.deauth_reason[idx]);
			}
		}
	}
	...
	if ((priv->plinkstats_cfg.filter & PLSTATS_FILTER_STA_LIST) &&
	    (priv->bss_role == MLAN_BSS_ROLE_UAP)) {
		sta_list = kzalloc(sizeof(mlan_ds_sta_list), GFP_ATOMIC);
		if (sta_list) {
			mlan_status status = woal_get_sta_list(priv, sta_list);
			if (status == MLAN_STATUS_SUCCESS) {
				ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
						"Number of STA = %d\n",
						sta_list->sta_count);
				for (i = 0; i < sta_list->sta_count; i++) {
					int rssi = (int)sta_list->info[i].rssi;
					if (rssi > 0x7f)
						rssi = -(256 - rssi);

					ptr += snprintf(ptr,
							MAX_LINK_STATS_BUF_SIZE,
							"STA %d information:\n",
							i + 1);
					ptr += snprintf(
						ptr, MAX_LINK_STATS_BUF_SIZE,
	...
	woal_broadcast_event(priv, (t_u8 *)stats_buf, strlen(stats_buf));
	kfree(stats_buf);
			       ...
			       bool is_update)
{
	...
	stats_buffer = kzalloc(MAX_LINK_STATS_BUF_SIZE, GFP_ATOMIC);
	if (!stats_buffer) {
		PRINTM(MERROR, "Failed to allocate stats buffer\n");
		goto done;
	}
		...
		if (priv->bss_role == MLAN_BSS_ROLE_STA) {
			...
			if (MLAN_STATUS_SUCCESS !=
			    woal_get_signal_info(priv, MOAL_IOCTL_WAIT,
						 &signal)) {
				PRINTM(MERROR,
				       "Error getting signal information\n");
				goto done;
			}
		}
	...
	if (is_reset) {
		...
		snprintf(stats_buffer, 4096,
			 "=== PLINKSTATS RESET - BASELINE ESTABLISHED ===\n");
		woal_update_perfmatrix_buffer(priv, stats_buffer);
		goto done;
	}
	...
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_TX_BYTES) {
		ptr += snprintf(
			...
			"\nNum_tx_bytes = %lu, \t Num_tx_packets = %lu\n",
			priv->plinkstats.tx_bytes, priv->plinkstats.tx_packets);
		PRINTM(MLSTATS, "Num_tx_bytes = %lu, \t Num_tx_packets = %lu\n",
		       priv->plinkstats.tx_bytes, priv->plinkstats.tx_packets);
	}
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_RX_BYTES) {
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				...
				priv->plinkstats.rx_bytes,
				priv->plinkstats.rx_packets);
		PRINTM(MLSTATS, "Num_rx_bytes = %lu, \t Num_rx_packets = %lu\n",
		       priv->plinkstats.rx_bytes, priv->plinkstats.rx_packets);
	}
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_CHLOAD) {
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
				"Channel Load = %d%%, noise = %d\n",
		...
		if (priv->plinkstats.deauth_count) {
			...
			for (idx = 0; idx < priv->plinkstats.deauth_count;
				...
				PRINTM(MLSTATS,
				       "Station deauthed " MACSTR
				       ", reason code = 0x%x\n",
				       MAC2STR(priv->plinkstats.deauth_mac[idx]),
				       priv->plinkstats.deauth_reason[idx]);
			}
			priv->plinkstats.deauth_count = 0;
		}
	...
	if ((priv->plinkstats_cfg.filter & PLSTATS_FILTER_SIGNAL) &&
		...
		ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE, "SNR = %d dB\n",
	...
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_STA_LIST) {
		...
		for (sta_idx = 0; sta_idx < sta_list->sta_count; sta_idx++) {
			ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
					"STA %d information:\n", sta_idx + 1);
			...
			ptr += snprintf(
				ptr, MAX_LINK_STATS_BUF_SIZE,
				"MAC Address: " MACSTR "\n",
				MAC2STR(sta_list->info[sta_idx].mac_address));
			PRINTM(MLSTATS, "MAC Address: " MACSTR "\n",
			       MAC2STR(sta_list->info[sta_idx].mac_address));
			ptr += snprintf(
				ptr, MAX_LINK_STATS_BUF_SIZE,
				"Power mgmt status: %s\n",
				(sta_list->info[sta_idx].power_mgmt_status ==
				 0) ?
					"active" :
					"power save");
			PRINTM(MLSTATS, "Power mgmt status: %s\n",
				...
				0) ?
				       ...
				       "power save");
			ptr += snprintf(
				ptr, MAX_LINK_STATS_BUF_SIZE, "Mode: %s\n",
				(sta_list->info[sta_idx].bandmode == BAND_B) ?
					"11b," :
	...
	/* Update perfmatrix with complete stats */
	if (strlen(stats_buffer) > 0) {
		PRINTM(MERROR, "Updating perfmatrix with %d bytes\n",
		       (int)strlen(stats_buffer));
		woal_update_perfmatrix_buffer(priv, stats_buffer);
	}
	if (priv->plinkstats_cfg.netlink_evt == MTRUE)
		woal_send_linkstats_event(priv);
...
static void woal_cleanup_module(void)
{
	...
	for (index = 0; index < MAX_MLAN_ADAPTER; index++) {
		...
		for (i = 0; i < handle->priv_num; i++) {
#ifdef MAC80211_SUPPORT
			if (handle->priv[i] && handle->priv[i]->bss_type ==
						       MLAN_BSS_TYPE_MAC80211) {
				if (handle->priv[i]->mac80211_registered) {
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
					if (handle->priv[i]
						    ->mac80211_mode_monitor) {
						woal_set_net_monitor(
							handle->priv[i],
							MOAL_IOCTL_WAIT, MFALSE,
							0, NULL);
						handle->priv[i]
							->mac80211_mode_monitor =
							MFALSE;
					...
					ieee80211_unregister_hw(
						handle->priv[i]->hw);
					handle->priv[i]->mac80211_registered =
						MFALSE;
				}
				continue;
			}
