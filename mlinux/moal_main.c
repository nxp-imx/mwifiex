			       ...
			       bool is_update)
{
	...
	if (is_update) {
		...
		priv->plinkstats.country_code[0] =
			priv->phandle->country_code[0];
		priv->plinkstats.country_code[1] =
	...
	if (priv->plinkstats_cfg.filter & PLSTATS_FILTER_REGION) {
		if (priv->plinkstats.country_code[0] &&
		    priv->plinkstats.country_code[1]) {
			ptr += snprintf(ptr, MAX_LINK_STATS_BUF_SIZE,
					"Region = %c%c\n",
					priv->plinkstats.country_code[0],
	...
	woal_cfg80211_vendor_event_plinkstats(priv);
