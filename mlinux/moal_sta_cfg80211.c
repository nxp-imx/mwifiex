...
#define delta64(later, earlier)                                                \
	((later >= earlier) ? later - earlier : (t_u64)(-1) - earlier + later)
				       ...
				       t_u8 wait_option)
{
	...
	if (c) {
		...
		if (status != MLAN_STATUS_SUCCESS) {
			ret = MLAN_STATUS_FAILURE;
			PRINTM(MERROR,
			       "Error setting channel attributes in the FW\n");
			goto done;
		}
	}
			   ...
			   struct regulatory_request *request)
{
	...
	case NL80211_REGDOM_SET_BY_CORE:
		...
		if (MTRUE == is_cfg80211_special_region_code(region)) {
			PRINTM(MCMND,
			       "WOAL: Ignore country=00; initiator=CORE\n");
			LEAVE();
#if CFG80211_VERSION_CODE < KERNEL_VERSION(3, 9, 0)
			return ret;
#else
...
#endif
			PRINTM(MEVENT,
			       "HostMlme: Drop disconnect notify, already disconnected\n");
			LEAVE();
			return;
		}
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(6, 7, 0)
