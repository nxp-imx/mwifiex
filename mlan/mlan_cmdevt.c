...
static mlan_status wlan_dnld_sleep_confirm_cmd(mlan_adapter *pmadapter)
{
	...
	if (pmbuf) {
		...
		if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_STA
		    ...
		    || GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_MAC80211
#endif
		) {
...
mlan_status wlan_adapter_get_hw_spec(pmlan_adapter pmadapter)
{
	...
	if (pmadapter->dpd_data_len == UNKNOW_DPD_LENGTH) {
	...
	} else if (pmadapter->dpd_data_len == NONE_DPD_LENGTH) {
		ret = wlan_prepare_cmd(priv, HostCmd_CMD_CFG_DATA,
				       HostCmd_ACT_GEN_GET, NONE_TYPE_DPD,
				       MNULL, MNULL);
		if (ret) {
			ret = MLAN_STATUS_FAILURE;
			goto done;
				...
				t_u16 cmd_action, t_void *pdata_buf)
{
	...
	if (cmd_action == HostCmd_ACT_GEN_SET) {
		...
		tsp_config->high_thrshld = wlan_cpu_to_le32(cfg->high_thrshld);
		tsp_config->low_thrshld = wlan_cpu_to_le32(cfg->low_thrshld);
		tsp_config->reg_cau_val = wlan_cpu_to_le32(cfg->reg_cau_val);
		tsp_config->duty_cyc_step =
			wlan_cpu_to_le32(cfg->duty_cyc_step);
		tsp_config->duty_cyc_min = wlan_cpu_to_le32(cfg->duty_cyc_min);
		...
		for (rfu = 0; rfu < MAX_RFUS; rfu++) {
			for (rpath = 0; rpath < MAX_PATHS; rpath++) {
				tsp_config->reg_rfu_val[rfu][rpath] =
					wlan_cpu_to_le32(
