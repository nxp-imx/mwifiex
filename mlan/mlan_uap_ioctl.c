					    ...
					    pmlan_ioctl_req pioctl_req)
{
	...
	memcpy_ext(pmadapter, (t_u8 *)&phecap->rx_mcs_80,
		   (t_u8 *)&phw_hecap->rx_mcs_80, sizeof(phecap->rx_mcs_80),
		   sizeof(phw_hecap->rx_mcs_80));
	memcpy_ext(pmadapter, (t_u8 *)&phecap->tx_mcs_80,
		   (t_u8 *)&phw_hecap->tx_mcs_80, sizeof(phecap->tx_mcs_80),
		   sizeof(phw_hecap->tx_mcs_80));
...
mlan_status wlan_ops_uap_ioctl(t_void *adapter, pmlan_ioctl_req pioctl_req)
{
	...
	case MLAN_IOCTL_MISC_CFG:
		...
		if (misc->sub_command == MLAN_OID_MISC_RANDOM_SN_CONFIG)
			status = wlan_misc_ioctl_random_sn(pmadapter,
							   pioctl_req);
