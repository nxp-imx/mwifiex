...
static mlan_operations mlan_mac80211_ops = {
	/* init cmd handler */
	wlan_ops_mac80211_init_cmd,
	/* ioctl handler */
	wlan_ops_mac80211_ioctl,
	/* cmd handler */
	wlan_ops_mac80211_prepare_cmd,
	/* cmdresp handler */
	wlan_ops_mac80211_process_cmdresp,
	/* rx handler */
	wlan_ops_mac80211_process_rx_packet,
	/* Event handler */
	wlan_ops_mac80211_process_event,
	/* txpd handler */
	wlan_ops_mac80211_process_txpd,
	/* BSS role: STA */
	MLAN_BSS_ROLE_MAC80211,
};
#endif
...
/**
 *  @brief This function checks if all necessary callbacks are set.
 *
 *  @param pmadapter  A pointer to mlan_adapter structure
 *  @param pcb        A pointer to mlan_callbacks structure
 *
 ...
 */
static mlan_status mlan_check_adapter_callbacks(pmlan_adapter pmadapter,
						pmlan_callbacks pcb)
{
	if (!pcb->moal_get_hw_spec_complete || !pcb->moal_init_fw_complete ||
	    !pcb->moal_shutdown_fw_complete ||
	    !pcb->moal_send_packet_complete || !pcb->moal_recv_packet ||
	    !pcb->moal_recv_event || !pcb->moal_ioctl_complete) {
		return MLAN_STATUS_FAILURE;
	}

#if defined(PCIE)
	if (!IS_USB(pmadapter->card_type)) {
		if (!pcb->moal_write_reg || !pcb->moal_read_reg ||
		    !pcb->moal_alloc_mlan_buffer ||
		    !pcb->moal_free_mlan_buffer) {
			return MLAN_STATUS_FAILURE;
		}
	}
#endif

	if (!pcb->moal_get_vdll_data) {
		return MLAN_STATUS_FAILURE;
	}

	if (!pcb->moal_write_data_sync || !pcb->moal_read_data_sync ||
	    !pcb->moal_mfree || !pcb->moal_memcpy || !pcb->moal_memcpy_ext ||
	    !pcb->moal_memcmp || !pcb->moal_get_system_time ||
	    !pcb->moal_init_timer || !pcb->moal_free_timer ||
	    !pcb->moal_get_boot_ktime || !pcb->moal_start_timer ||
	    !pcb->moal_stop_timer || !pcb->moal_init_lock ||
	    !pcb->moal_free_lock || !pcb->moal_spin_lock ||
	    !pcb->moal_spin_unlock || !pcb->moal_hist_data_add ||
	    !pcb->moal_updata_peer_signal || !pcb->moal_do_div) {
		return MLAN_STATUS_FAILURE;
	}
...
mlan_status mlan_register(pmlan_device pmdevice, t_void **ppmlan_adapter)
{
	...
	if (!pmdevice || !ppmlan_adapter || !pmdevice->callbacks.moal_print) {
		return MLAN_STATUS_FAILURE;
	}
	...
	if (!pmdevice->callbacks.moal_malloc ||
		...
		PRINTM(MERROR, "MLAN device callbacks missed\n");
	...
	if (mlan_check_adapter_callbacks(pmadapter, pcb) !=
	    MLAN_STATUS_SUCCESS) {
		PRINTM(MERROR, "MLAN adapter callbacks missed\n");
		ret = MLAN_STATUS_FAILURE;
		goto error;
	}
	...
	if (IS_PCIE(pmadapter->card_type)) {
		...
		if (!pcb->moal_malloc_consistent ||
		    !pcb->moal_mfree_consistent || !pcb->moal_map_memory ||
		    !pcb->moal_unmap_memory) {
			PRINTM(MERROR, "MLAN PCIE callback missed\n");
			ret = MLAN_STATUS_FAILURE;
			goto error;
		}
		PRINTM(MMSG,
...
mlan_status mlan_unregister(t_void *padapter)
{
	...
	MASSERT(padapter);
	if (!padapter) {
		PRINTM(MERROR, "%s: null adpater pointer\n", __FUNCTION__);
		return MLAN_STATUS_FAILURE;
	}
...
mlan_status mlan_send_packet(t_void *padapter, pmlan_buffer pmbuf)
{
	...
	if (!padapter || !pmbuf) {
		PRINTM(MERROR, "%s: null pointer, adapter=%p, mbuf=%p\n",
		       __FUNCTION__, padapter, pmbuf);
		LEAVE();
		return MLAN_STATUS_FAILURE;
	}

	MASSERT(pmbuf->bss_index < pmadapter->priv_num);
	if (pmbuf->bss_index >= pmadapter->priv_num) {
		PRINTM(MERROR, "%s: Unexpected bss_index=%d (priv_num=%d)\n",
		       __FUNCTION__, pmbuf->bss_index, pmadapter->priv_num);
		LEAVE();
		return MLAN_STATUS_FAILURE;
	}
...
#ifdef MAC80211_SUPPORT
	if (pmpriv->bss_type == MLAN_BSS_TYPE_MAC80211) {
...
#ifdef ICMP_VIA_BYPASS_TXQ
	      /** Send ICMP packet via bypass_txqueue to reduce long ping
		 latency */
