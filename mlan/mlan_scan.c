			...
			case MU_EDCA_PARAM_SET:
				...
				pbss_entry->pmuedca_ie =
					(IEEEtypes_MUEDCAParamSet_t *)
						pcurrent_ptr;
				pbss_entry->muedca_offset = (t_u16)(
					pcurrent_ptr - pbss_entry->pbeacon_buf);
				break;
			case MBSSID_CONFIG:
				pbss_entry->pmbssid_config =
					(IEEEtypes_MBSSID_Config_t *)
						pcurrent_ptr;
				pbss_entry->mbssid_config_offset = (t_u16)(
					pcurrent_ptr - pbss_entry->pbeacon_buf);
				break;
			default:
				break;
...
static t_void wlan_scan_process_results(mlan_private *pmpriv)
{
	...
	if (pmpriv->bss_role == MLAN_BSS_ROLE_STA) {
		(void)wlan_11d_prepare_dnld_domain_info_cmd(pmpriv);
	}
					      ...
					      t_u8 **pbeacon_info, t_u32 ie_len)
{
	...
	while (bytes_left >= 2) {
		...
		case VENDOR_SPECIFIC_221:
			...
			if (!memcmp(pmadapter, pvendor_ie->vend_hdr.oui,
				...
				pnew_entry->pwpa_ie =
					(IEEEtypes_VendorSpecific_t *)
						pcurrent_ptr;
				pnew_entry->wpa_offset = (t_u16)(
					pcurrent_ptr - pnew_entry->pbeacon_buf);
		...
		case EXTENSION:
			...
			case HE_CAPABILITY:
				pnew_entry->phe_cap =
					(IEEEtypes_HECap_t *)pcurrent_ptr;
				pnew_entry->he_cap_offset = (t_u16)(
					pcurrent_ptr - pnew_entry->pbeacon_buf);
				DBG_HEXDUMP(
					MINFO, "HECAP_IE:",
			...
			case HE_OPERATION:
				...
				pnew_entry->he_oprat_offset = (t_u16)(
					pcurrent_ptr - pnew_entry->pbeacon_buf);
				DBG_HEXDUMP(
					MINFO, "HEOPER_IE:",
			...
			case MU_EDCA_PARAM_SET:
				pnew_entry->pmuedca_ie =
					(IEEEtypes_MUEDCAParamSet_t *)
						pcurrent_ptr;
				pnew_entry->muedca_offset = (t_u16)(
					pcurrent_ptr - pnew_entry->pbeacon_buf);
			...
			case MBSSID_CONFIG:
				pnew_entry->pmbssid_config =
					(IEEEtypes_MBSSID_Config_t *)
						pcurrent_ptr;
				pnew_entry->mbssid_config_offset = (t_u16)(
					pcurrent_ptr - pnew_entry->pbeacon_buf);
	...
	pcb->moal_mfree(pmadapter->pmoal_handle, (t_u8 *)pbuf);
	LEAVE();
	return ret;
}
