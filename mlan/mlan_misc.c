...
sta_node *wlan_get_station_entry(mlan_private *priv, t_u8 *mac)
{
	...
	ENTER();
	pmadapter->callbacks.moal_spin_lock(priv->adapter->pmoal_handle,
					    priv->sta_list.plock);

	if (!mac) {
		LEAVE();
		pmadapter->callbacks.moal_spin_unlock(
			priv->adapter->pmoal_handle, priv->sta_list.plock);
		return MNULL;
	}
	...
	LEAVE();
	pmadapter->callbacks.moal_spin_unlock(priv->adapter->pmoal_handle,
					      priv->sta_list.plock);
	return MNULL;
}
...
/**
 *  @brief Send uap station deauth reason code and mac address
 *
 *  @param reason_code    Deauth reason code
 *  @param mac_addr       A pointer to station mac address
 *
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_send_uap_sta_deauth_event(mlan_private *priv,
					   t_u16 reason_code, t_u8 *mac_addr)
{
	...
	t_u32 event_data_len = sizeof(reason_code) + MLAN_MAC_ADDR_LENGTH;

	ret = pcb->moal_malloc(pmadapter->pmoal_handle,
			       sizeof(mlan_event) + event_data_len,
			       MLAN_MEM_DEF, &event_buf);
	if ((ret != MLAN_STATUS_SUCCESS) || !event_buf) {
		PRINTM(MERROR, "Failed to allocate memory for deauth event\n");
		return MLAN_STATUS_FAILURE;
	}

	pevent = (mlan_event *)event_buf;
	pevent->bss_index = priv->bss_index;
	pevent->event_id = MLAN_EVENT_ID_DRV_UAP_STA_DEAUTH;
	pevent->event_len = event_data_len;

	reason_code = wlan_cpu_to_le16(reason_code);
	memcpy_ext(priv->adapter, pevent->event_buf, &reason_code,
		   sizeof(reason_code), sizeof(reason_code));
	memcpy_ext(priv->adapter, pevent->event_buf + sizeof(reason_code),
		   mac_addr, MLAN_MAC_ADDR_LENGTH, MLAN_MAC_ADDR_LENGTH);
					   ...
					   t_u32 payload_len, RxPD *prx_pd)
{
	...
	case SUBTYPE_DEAUTH:
		...
		if (priv->uap_host_based & UAP_FLAG_HOST_MLME) {
			if (!memcmp(pmadapter, pieee_pkt_hdr->addr3,
				    priv->curr_addr, MLAN_MAC_ADDR_LENGTH)) {
				t_u16 reason_code = wlan_le16_to_cpu(
					*(t_u16 *)(payload +
						   sizeof(wlan_802_11_header)));
				PRINTM_NETINTF(MMSG, priv);
				PRINTM(MMSG,
				       ...
				       MAC2STR(pieee_pkt_hdr->addr2),
				       reason_code);
				wlan_send_uap_sta_deauth_event(
					priv, reason_code,
					pieee_pkt_hdr->addr2);
			}
		}
				   ...
				   pmlan_ioctl_req pioctl_req)
{
	...
	if (pioctl_req->action == MLAN_ACT_GET)
		misc->param.multi_ap.multi_ap_flag = pmpriv->multi_ap_flag;
		...
		pmpriv->multi_ap_flag = misc->param.multi_ap.multi_ap_flag;
	}
			       ...
			       mef_cfg_data *pmef)
{
	...
	for (i = 0, pentry = pmef->pentry; i < pmef->entry_num; i++) {
		...
		if (!wlan_secure_add((void *)&pentry, sizeof(mef_entry_t),
				     (void *)&pentry, TYPE_PTR))
			break;
	}
...
/**
 *  @brief get/set ProbeReq random SN config
 *
 *  @param pmadapter    A pointer to mlan_adapter structure
 *  @param pioctl_req   Pointer to the IOCTL request buffer
 *
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_misc_ioctl_random_sn(pmlan_adapter pmadapter,
				      mlan_ioctl_req *pioctl_req)
{
	mlan_ds_misc_cfg *misc = MNULL;
	mlan_status ret = MLAN_STATUS_SUCCESS;
	...
	misc = (mlan_ds_misc_cfg *)pioctl_req->pbuf;
	if (pioctl_req->action == MLAN_ACT_SET) {
		pmadapter->probe_req_rand_sn = misc->param.random_sn.value;
	}
	misc->param.random_sn.value = pmadapter->probe_req_rand_sn;
