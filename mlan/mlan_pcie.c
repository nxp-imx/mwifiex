	...
	for (i = 0; i < pmadapter->pcard_pcie->txrx_bd_size; i++) {
		...
		if (pmadapter->pcard_pcie->reg->use_adma) {
			...
			if (dummy_pmbuf) {
				...
				PRINTM(MMSG, "%s: ERR dummy_pmbuf is NULL\n",
				       __func__);
...
done_unmap:
	...
	if (pmadapter->pcard_pcie->reg->use_adma && padma_bd_buf) {
		mlan_buffer *dummy_pmbuf = pmadapter->pcard_pcie->dummy_tx_buf;
		if (dummy_pmbuf) {
			padma_bd_buf->paddr =
				wlan_cpu_to_le64(dummy_pmbuf->buf_pa);
			padma_bd_buf->len = ALIGN_SZ(
...
mlan_status wlan_alloc_pcie_dummy_tx_buf(pmlan_adapter pmadapter)
{
	...
	PRINTM(MMSG, "Dummy mlan_buffer's mapped memory pmbuf->buf_pa: 0x%p\n",
	       pmbuf->buf_pa);
