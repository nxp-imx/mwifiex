...
{
	uint32_t hi = read32_be(buf), lo = read32_be(buf + 4);
...
{
	uint32_t hi = read32_le(buf + 4), lo = read32_le(buf);
	return ((uint64_t)hi) << 32 | lo;
}
...
static inline void xor_b8(uint8_t *out, const uint8_t *in, uint8_t b8,
			  size_t len)
{
...
static inline void xor_bb(uint8_t *out, const uint8_t *x, const uint8_t *y,
			  size_t len)
{
	for (size_t i = 0; i < len; i++)
		out[i] = x[i] ^ y[i];
...
/** Select the ith entry from the given table of n values, in a side
 * channel-silent way. */
static inline uint32_t select_u32(uint32_t i, volatile const uint32_t *tab,
				  uint32_t n)
{
	uint32_t r = 0;
...
/** Select the ith entry from the given table of n values, in a side
 * channel-silent way. */
static inline uint8_t select_u8(uint32_t i, volatile const uint8_t *tab,
				uint32_t n)
{
	uint8_t r = 0;
			       ...
			       volatile const uint8_t *tab, uint32_t n)
{
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		mask = mask_u8(*a, i);
		ra = (ra & ~mask) | (item & mask);
		mask = mask_u8(*b, i);
		rb = (rb & ~mask) | (item & mask);
		mask = mask_u8(*c, i);
		rc = (rc & ~mask) | (item & mask);
		mask = mask_u8(*d, i);
		rd = (rd & ~mask) | (item & mask);
	}
...
static inline void select_xor128(uint32_t out[4], const uint32_t if0[4],
				 const uint32_t if1[4], uint8_t bit)
{
 ...
 *  to the right. */
static inline void copy_bytes_unaligned(uint8_t *out, const uint8_t *in,
