...
static void sha256_update_block(void *vctx, const uint8_t *inp)
{
	...
	for (size_t t = 0; t < 64; t++) {
		...
		if (t < 16) {
			...
			Wt = SSIG1(W[(t - 2) % 16]) + W[(t - 7) % 16] +
			     SSIG0(W[(t - 15) % 16]) + W[(t - 16) % 16];
			W[t % 16] = Wt;
		}
...
void cf_sha256_digest(const cf_sha256_context *ctx,
...
void cf_sha256_digest_final(cf_sha256_context *ctx,
			    uint8_t hash[CF_SHA256_HASHSZ])
{
	...
	size_t padbytes =
		CF_SHA256_BLOCKSZ - ((digested_bytes + 8) % CF_SHA256_BLOCKSZ);
...
void cf_sha224_digest(const cf_sha256_context *ctx,
...
void cf_sha224_digest_final(cf_sha256_context *ctx,
			    uint8_t hash[CF_SHA224_HASHSZ])
{
	uint8_t full[CF_SHA256_HASHSZ];
...
const cf_chash cf_sha224 = {.hashsz = CF_SHA224_HASHSZ,
...
const cf_chash cf_sha256 = {.hashsz = CF_SHA256_HASHSZ,
			    .blocksz = CF_SHA256_BLOCKSZ,
