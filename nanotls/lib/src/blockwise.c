				   ...
				   cf_blockwise_in_fn process_final, void *ctx)
{
	...
	uassert(partial && *npartial < nblock);
	uassert(inp || !nbytes);
	uassert(process && ctx);
			  ...
			  size_t nbytes, cf_blockwise_in_fn process, void *ctx)
{
	...
	default:
		...
		/* If the middle and last bytes differ, then process the last
		 * byte separately. Otherwise, just extend the middle block
		 * size. */
		if (lbyte != mbyte) {
