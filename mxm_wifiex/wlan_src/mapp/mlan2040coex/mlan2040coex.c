 ...
 */
static int
get_netlink_num(char *dev_name, int dev_index)
{
	FILE *fp = NULL;
	int netlink_num = -1;
	char str[64];
 ...
 */
int
open_netlink(char *dev_name, int dev_index)
{
	int sk = -1;
	struct sockaddr_nl src_addr;
	int netlink_num = 0;

	netlink_num = get_netlink_num(dev_name, dev_index);
	if (netlink_num < 0) {
...
main(int argc, char *argv[])
{
	...
	int dev_index = -1;	      /** initialise with -1 to open multiple NETLINK Sockets */
	...
	nl_sk = open_netlink(dev_name, dev_index);
