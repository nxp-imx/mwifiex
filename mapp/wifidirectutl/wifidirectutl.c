					  ...
					  t_u16 *ie_len_wps)
{
	...
	t_u8 wifidirect_client_dev_count = 0, wifidirect_client_dev_index = 0,
	     temp8 = 0;
	wifidirect_client_dev_info *wifidirect_client_dev_info_list = NULL;
	...
	wifidirect_client_dev_info_list = (wifidirect_client_dev_info *)malloc(
		MAX_SECONDARY_DEVICE_COUNT *
		sizeof(wifidirect_client_dev_info));
	if (!wifidirect_client_dev_info_list) {
		printf("ERR:Cannot allocate memory for wifidirect_client_dev_info_list\n");
		goto done;
	}
	memset(wifidirect_client_dev_info_list, 0,
	       (MAX_SECONDARY_DEVICE_COUNT *
		sizeof(wifidirect_client_dev_info)));
	...
	if (extra)
		free(extra);
	if (wifidirect_client_dev_info_list)
		free(wifidirect_client_dev_info_list);
	if (args)
		free(args);
	return;
}
