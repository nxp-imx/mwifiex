...
static int process_probe_req_random_sn(int argc, char *argv[]);
...
struct command_node command_list[] = {
	...
	{"agcs", process_agcs},
#endif
	{"random_sn", process_probe_req_random_sn},
};
...
static int process_11axcmdcfg(int argc, char *argv[])
{
	...
	t_u32 data_len = 0;
	char *llde_cfg_data = NULL;
	char file_name[100];
	int ret = MLAN_STATUS_SUCCESS;

	llde_cfg_data = (char *)malloc(BUFFER_LENGTH);
	if (!llde_cfg_data) {
		printf("ERR:Cannot allocate memory for llde_cfg_data!\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	memset(llde_cfg_data, 0, BUFFER_LENGTH);
...
#define MAX_CFG_ID_SIZE 2
	if (strcmp(argv[3], "obss_pd_offset") == 0) {
		...
		if (argc == 5) {
			 ...
			 * firmware*/
			data_len = snprintf(llde_cfg_data, BUFFER_LENGTH, "%s",
					    CMD_NXP);
			data_len += snprintf(llde_cfg_data + data_len,
					     BUFFER_LENGTH - data_len, "%s",
					     argv[2]);
			data_len += snprintf(llde_cfg_data + data_len,
					     BUFFER_LENGTH - data_len, "%d",
...
static int process_mefcfg(int argc, char *argv[])
{
	char cmdname[256], *pos = NULL, *line = NULL;
	size_t size = 256;
	...
	if (!line) {
		printf("ERR:malloc failed for line!\n");
		return -ENOMEM;
	}
	...
	while ((pos = mlan_config_get_line(fp, line, size, &ln))) {
		if (strcmp(pos, cmdname) == 0) {
			cmdname_found = 1;
			snprintf(cmdname, sizeof(cmdname), "Criteria=");
			name_found = 0;
			while ((pos = mlan_config_get_line(fp, line, size,
							   &ln))) {
...
static int process_mgmtframetx(int argc, char *argv[])
{
	...
	char **args = NULL, *pos = NULL, mac_addr[21] = {0};
	t_u8 peer_mac[ETH_ALEN];
	...
	args = malloc(sizeof(char *) * MAX_ARG_SIZE);
	if (!args) {
		printf("ERR:Cannot allocate memory for args\n");
		goto done;
	}
			      ...
			      region_chan_pwr_tbl *rgchnpwr_tbl)
{
	...
	fr = fread(tmp_buf, sizeof(t_u8), file_size, src_fp);
	if (fr == 0) {
...
static int process_cwmode(int argc, char *argv[])
{
	...
	if (cmd_len >= remaining) {
		printf("ERR: CMD_NXP too long for buffer\n");
		free(buffer);
		return MLAN_STATUS_FAILURE;
	}
	memcpy(pos, CMD_NXP, cmd_len);
	pos += cmd_len;
	remaining -= cmd_len;

	// Copy argv[2]
	if (arg_len >= remaining) {
		printf("ERR: argv[2] too long for buffer\n");
		free(buffer);
		return MLAN_STATUS_FAILURE;
	}
	memcpy(pos, argv[2], arg_len);
	pos += arg_len;
	remaining -= arg_len;
...
static void receive_dot11_packet(char *ifName, int protocol, int verbose)
{
	...
	buf = (t_u8 *)malloc(BUF_SIZ);
	if (!buf) {
		printf("malloc failed for buf");
		return;
	}
...
static int process_probe_req_random_sn(int argc, char *argv[])
{
	...
	if (argc < 3 || argc > 4) {
		printf("Err: Invalid number of arguments\n");
		printf("Usage: ./mlanutl <interface> random_sn [value]\n");
		return MLAN_STATUS_FAILURE;
	}
	if ((argc == 4) && (*argv[3] != '0') && (*argv[3] != '1')) {
		printf("Err: Invalid input argument of [value]!\n");
		printf("Currently, we only support 0 and 1 for [value]\n");
		return MLAN_STATUS_FAILURE;
	}
	...
	if (argc == 4) {
		memcpy((char *)pos, " ", strlen(" "));
		pos += 1;
		strncpy((char *)pos, argv[3], strlen(argv[3]));
	}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: random_sn fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
