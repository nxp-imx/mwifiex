...
static int process_keep_connect(int argc, char *argv[]);
#endif
...
#ifdef FORCE_KEEP_CONNECT
	{"keep_connect", process_keep_connect},
#endif
	{"csi", process_csi_cmd},
	{"dmcs", process_dmcs},
...
#endif /*defined(CLOUD_KEEP_ALIVE) || defined(CLOUD_KEEP_ALIVE_ACK)||          \
	  !defined(MLANUTL_LITE)*/
...
static int process_keep_connect(int argc, char *argv[])
{
	...
	if (argc != 4) {
		printf("Err: Invalid number of arguments\n");
		printf("Usage: ./mlanutl <interface> keep_connect [Enable/Disable]\n");
		return MLAN_STATUS_FAILURE;
	}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: keep_connect fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
