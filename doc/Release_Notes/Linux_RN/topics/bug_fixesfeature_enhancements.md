...
## Firmware version 17.92.5.p3 to 17.92.5.p9

**Wi-Fi**

- Wake On Wireless Feature

## Firmware version 17.92.5.p9 to 17.92.5.p11

**Wi-Fi**

- In RF Test Mode Tx tests, the device is unable to transmit Tx Frame and Tx Continuous Wave modes. Manufacturing software can be used for validation.

## Firmware version 17.92.5.p11 to 17.92.1.p116.1

**Wi-Fi**
- Low TCP/UDP Tx \(by ~80%\) and TCP/UDP Rx \(by ~70%\) throughput is observed for Internal STA mode on MAC2 interface in BGN20 mode with Netgear R6200 AP.
- Low UDP Tx \(20-25%\) throughput observed on HE\-80 MHz Band For All Securities.
- Internal-AP mode the data-rate drops to 0 Mbps and does not recover when TCP Bidirectional test is run in HE80/WPA2 mode after ~2 hours.
- P2P GO on/off stress test fails and DUT stops responding after ~1 hour.
...
**Wi-Fi**
- Wake-up card timeout is seen when performing suspend and resume stress test with i.MX 8 host.
- Command timeout is seen when performing connection and disconnection test in a loop with external AP during addition of block ack requests.
...
- During the penetration testing of the ECU under test, a buffer overflow vulnerability was found in the Wi-Fi driver.
...
**Wi-Fi**
- Fixed kernel panic fetal error due to connecting the DUT STA to a long SSID Ex-AP.
...
In DUT STA mode, the Reassociation response from the External Access Point was dropped after the acknowledgement was sent. Resulted in another Reassociation request, ultimately leading to Deauthentication from the External Access Point.
Firmware crash occurrence in the 2.4 GHz band during a roaming stress test scenario.
