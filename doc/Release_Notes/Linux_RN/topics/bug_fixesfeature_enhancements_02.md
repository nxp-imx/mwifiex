...
## Firmware version 16.92.10.p208 to 16.92.10.p211

**Wi-Fi**
- Fixed Mobile AP start issue on switching bands

**Bluetooth**
- Fix for Sniff Subrate command processing which resulted in command queue that caused Bluetooth to restart.
- Fix for ACL link disconnection due to DUT not responding to LMP\_switch\_req.

**Coexistence**
- Fix for Wi-Fi Link loss during UDP RX + Bluetooth Inquiry and Wi-Fi deauth during Bluetooth HFP coexistence scenarios

## Firmware version 16.92.10.p211 to 16.92.10.p213

**Wi-Fi**
- Fix for Wi-Fi Fragment and Forge Vulnerabilities[Engineering Bulletin - Wi-Fi vulnerability](references.md)

**Bluetooth**
- Fix for ANSSI Vulnerabilities [Engineering Bulletin - Bluetooth vulnerability](references.md)

## Firmware version 16.92.10.p213 to 16.92.10.p213.2

**Wi-Fi**
- Added support for 40 MHz band in 2.4 GHz BGN mode for AP and STA

## Firmware version 16.92.10.p213.2 to 16.92.10.p213.4
...
**Wi-Fi**
- If the DUT is in Tx-mode, a Wakeup-Card timeout is observed causing the device to Hang/Crash.
- Link Lost observed during roaming even with good RSSI

**Bluetooth**
- Bluetooth-only firmware initialization is failing when it is downloaded and initialized after Wi-Fi-only firmware initialization.

## Firmware version 16.92.21.p119.3 to 16.92.21.p137.4

**Bluetooth**
- Sometimes, when the DUT is streaming A2DP data to another remote device, the DUT role switch request is failing to the first remote device.<br/>
...
**Wi-Fi**
- During the roaming test with Meraki APs, the DUT STA explicitly sends a deauthentication frame to the previous AP to clean the STA list cache of the AP.
...
- Failure to set the MAC address with an init config file.In the roaming test, the DUT failed to connect back to the previous AP due to association failure.
